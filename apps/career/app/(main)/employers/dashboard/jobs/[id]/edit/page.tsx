...
export default function EditJobPage() {
  ...
  const [state, formAction, pending] = useActionState(updateJobAction, { status: "idle" });
  ...
  useEffect(() => {
    fetch(`/api/jobs/${jobId}`)
      ...
      .then((data) => {
        setJob(data);
        setWomenFriendly(Boolean(data.women_friendly));
        setSalaryMin(String(data.salary_min ?? ""));
  ...
  if (loading) {
    return (
      <div className="max-w-4xl mx-auto space-y-6">
        <p className="text-sm text-[var(--muted-fg)]">Loading job...</p>
      </div>
    );
  }
  ...
  if (!job) {
    return (
      <div className="max-w-4xl mx-auto space-y-6">
        <p className="text-sm text-[var(--muted-fg)]">Job not found.</p>
        <Link href="/employers/dashboard/jobs" className="text-sm text-[var(--accent-color)] hover:underline">Back to Jobs</Link>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h2 className="text-xl font-semibold text-[var(--foreground)]">Edit Job</h2>
          <p className="text-sm text-[var(--muted-fg)] mt-1">Update the details for this job posting.</p>
        </div>
      </div>
      ...
      <div className="bg-white/40 backdrop-blur-md rounded-2xl border border-white/60 shadow-sm p-6 sm:p-8">
        ...
        <form action={formAction} id="edit-job-form" className="space-y-8">
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <select name="job_type" defaultValue={String(job.job_type ?? "")} className="w-full bg-white/50 border border-[var(--border)] rounded-lg px-4 py-2.5 text-sm text-[var(--foreground)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--primary)] transition-colors" required>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div>
                ...
                <p className="text-xs text-[var(--muted-fg)] mt-1">Existing saved salaries are annual INR.</p>
              </div>
              <div>
                ...
                <input
                  ...
                  aria-describedby="edit-job-salary-help"
                  className="w-full bg-white/50 border border-[var(--border)] rounded-lg px-4 py-2.5 text-sm text-[var(--foreground)] placeholder:text-[var(--faint-fg)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--primary)] transition-colors"
                  required
                />
              </div>
              <div>
                ...
                <input
                  ...
                  min={salaryMin || "0"}
                  step="1"
                  defaultValue={String(job.salary_max ?? "")}
                  aria-describedby="edit-job-salary-help"
                  className="w-full bg-white/50 border border-[var(--border)] rounded-lg px-4 py-2.5 text-sm text-[var(--foreground)] placeholder:text-[var(--faint-fg)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--primary)] transition-colors"
                  required
                />
            ...
            <p id="edit-job-salary-help" className="text-xs text-[var(--muted-fg)] -mt-3">
          ...
          <section className="space-y-6">
            ...
            <div>
              ...
              <textarea
                name="requirements"
                rows={4}
                defaultValue={Array.isArray(job.tags) ? (job.tags as string[]).join("\n") : ""}
          ...
          <div className="pt-6 flex items-center justify-end gap-4 border-t border-[var(--border)]">
            ...
            <button
              ...
              value="draft"
              disabled={pending}
              className="px-6 py-2.5 rounded-[4px] text-sm font-medium text-[var(--accent-color)] border border-[var(--accent-color)] hover:bg-[var(--accent-color)] hover:text-[var(--on-accent)] transition duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--primary)] disabled:opacity-50"
            >
            ...
            <button
              ...
              value="active"
              disabled={pending}
              className="bg-[var(--accent-color)] text-[var(--on-accent)] px-6 py-2.5 rounded-[4px] text-sm font-medium hover:bg-[var(--accent-dark)] transition duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-[var(--primary)] disabled:opacity-50"
            >
